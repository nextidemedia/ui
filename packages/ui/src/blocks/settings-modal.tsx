    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        <DialogPrimitive.Backdrop
          ...
          className={cn(
            "fixed inset-0 z-50 bg-black/65 backdrop-blur-xl data-open:animate-in data-open:fade-in-0 data-closed:animate-out data-closed:fade-out-0",
            overlayClassName
          )}
        />
        <DialogPrimitive.Popup
          ...
          className={cn(
            "fixed top-1/2 left-1/2 z-50 grid max-h-[min(45rem,calc(100vh-2rem))] w-[min(32.5rem,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 grid-rows-[auto_minmax(0,1fr)] overflow-hidden rounded-xl border border-nextide-line bg-background text-foreground shadow-[0_30px_90px_rgb(0_0_0/0.55)] outline-none data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-open:slide-in-from-bottom-2 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
            className,
            contentClassName
          )}
        >
  ...
  return (
    <div data-slot="settings-progress-action" className="grid gap-2">
      ...
      >
        ...
        <b>
          {stateLabel ??
            (active ? "Running" : clampedProgress === 100 ? "Done" : "Ready")}
        </b>
        <span
          aria-hidden="true"
