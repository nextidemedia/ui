...
function renderExportSessions(sessions: ExportSessionRow[]) {
  return (
    <DataLedger
            ...
            {sessions.map((row) => (
              <TableRow key={row.id} className="border-0 hover:bg-transparent">
                <TableCell className="rounded-l-lg border-y border-l border-nextide-line bg-background/25 whitespace-normal">
                  <span className="block min-w-0 truncate">{row.creator}</span>
                </TableCell>
                <TableCell className="border-y border-nextide-line bg-background/25 whitespace-normal text-muted-foreground">
                  <span className="block min-w-0 truncate">{row.window}</span>
                </TableCell>
